...
public class IndexBar extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        int textSize = (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 10, getResources().getDisplayMetrics());//默认的TextSize
        ...
        mPaintBg.setColor(Color.parseColor("#13A8A8"));
        //设置index触摸监听器
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        //只在绘制View之前测量，绘制完成后，调用onDraw之前不会再进行测量了
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mIndexDatas.size(); i++) {
            ...
            if (i==pressIndex) {
                canvas.drawCircle(mWidth >> 1, t + mGapHeight * i+baseline-mPaint.measureText(index) / 2,dp2px(14f) , mPaintBg);
            }
            canvas.drawText(index, (mWidth >> 1) - mPaint.measureText(index) / 2, t + mGapHeight * i + baseline, mPaint);//调用drawText，居中显示绘制index
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //滑动的时候调用invalidate，才回随着滑动改变调用onDraw方法，否则的话只会在点击后才调用onDraw方法。
                invalidate();
                break;
    ...
    private int getPosByTag(String tag) {
        ...
        for (int i = 0; i < mSourceDatas.size(); i++) {
            if (tag.equals(mSourceDatas.get(i).getBaseIndexTag())) {
                //使用RecyclerView的时候返回i + getHeaderViewCount()即可，不知道为啥这里使用XRecyclerView需要返回i + getHeaderViewCount()+2
                return i + getHeaderViewCount()+2;
            }
        }
        return -1;
    }
